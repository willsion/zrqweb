...
def Login(request):
    if request.method == 'POST':
        ...
        if not is_empty:
            return render_to_response('login.html',{'error':"数据不能为空.",'username':username if username else '','pwd':password if password else '' })
        ...
        if user is not None:
            ...
            return render_to_response('login.html',{'error':"用户名或密码输入错误.",'username':username,'pwd':password })
    else:
